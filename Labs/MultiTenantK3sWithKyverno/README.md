...
This lab provides step-by-step instructions for setting up a comprehensive multi-tenant Kubernetes environment using K3s. This guide assumes you've already reviewed the main project README and have the required prerequisites installed.

## Scenario

Your organization has many development groups, and they all use Kubernetes to deploy and expose their services. Instead of having each development team create, manage, secure, upgrade, and maintain their own cluster, your organization has decided to create one centralized cluster that can be managed and secured by your platform team. However, development groups should not have free reign in the whole cluster. They need to be isolated so one team cannot access another team's workloads. Likewise, their workloads should not be able to access or harm another team's workloads from an RBAC and network perspective. Finally, the development teams should only have access to a subset of cluster services within the cluster. This example lab shows a point in time representing an average of configurations Virtue Security has observed for organizations seeking this goal.
...
- **Security**: Kyverno policy enforcement with medium risk pod security standard
...
$ # Post Install setup
$ kubectl apply -f install/capsule-example.yaml
